...
class RepVGGStage(nn.Module):
    def __init__(self, in_ch, stage_ch, num_block, kernel_size=3, groups=1, conv_type="RepVGGBlock", sep_conv=False):
        ...
        for num in range(num_block):
            if num == 0:
                if conv_type == "DBBBlock" and in_ch != stage_ch:
                    LayerDict["Block{}".format(num)] = ConvModule(in_ch, stage_ch, groups=groups,
                                                                  kernel_size=kernel_size, stride=2, padding=1,
                                                                  norm_cfg=dict(type='BN', requires_grad=True))
                elif conv_type == "RepVGGBlock":
                    if sep_conv and in_ch != stage_ch:
                        LayerDict["Block{}".format(num)] = CONV_TYPE(in_ch, in_ch, groups=in_ch,
                                                                     kernel_size=kernel_size, stride=2)
                        LayerDict["pw"] = ConvModule(in_ch, stage_ch, groups=1, kernel_size=1, stride=1)
                        in_ch = stage_ch
                    else:
                        LayerDict["Block{}".format(num)] = CONV_TYPE(in_ch, stage_ch, groups=groups,
                                                                     kernel_size=kernel_size, stride=2)
                elif conv_type == "GhostBlock":
                    LayerDict["Block{}".format(num)] = CONV_TYPE(in_ch, stage_ch, kernel_size=kernel_size, stride=2)
                else:
                    LayerDict["Block{}".format(num)] = ConvModule(in_ch, stage_ch, groups=groups,
                                                                  kernel_size=kernel_size, stride=2, padding=1)
                continue

            if conv_type == "RepVGGBlock":
                LayerDict["Block{}".format(num)] = CONV_TYPE(stage_ch, stage_ch, groups=groups, kernel_size=kernel_size,
                                                             stride=1)
            elif conv_type == "GhostBlock":
                LayerDict["Block{}".format(num)] = CONV_TYPE(stage_ch, stage_ch, kernel_size=kernel_size, stride=1)
            else:
                LayerDict["Block{}".format(num)] = ConvModule(stage_ch, stage_ch, groups=groups,
...
class RepVGGNet(BaseBackbone):
    def __init__(self,
        ...
        for num_stages in range(self.stage_nums):
            group = stage_channels[num_stages] if sep_conv else 1
    ...
    def train(self, mode=True):
        super(RepVGGNet, self).train(mode)
...
#
# class RepVGGStage(nn.Module):
#     def __init__(self, in_ch, stage_ch, num_block, kernel_size=3, groups=1, conv_cfg=dict(type="DBBConv")):
#         super(RepVGGStage, self).__init__()
#         LayerDict = OrderedDict()
#
#         for num in range(num_block):
#             if num == 0:
#                 if conv_cfg["type"] == "DBBConv" and in_ch != stage_ch:
#                     LayerDict["Block{}".format(num)] = ConvModule(in_ch, stage_ch, groups=groups, kernel_size=kernel_size, stride=2, padding=1,
#                                norm_cfg=dict(type='BN', requires_grad=True))
#                 else:
#                     LayerDict["Block{}".format(num)] = ConvModule(in_ch, stage_ch, groups=groups, kernel_size=kernel_size, stride=2, conv_cfg=conv_cfg)
#                 continue
#             LayerDict["Block{}".format(num)] = ConvModule(stage_ch, stage_ch, groups=groups, kernel_size=kernel_size, stride=1, conv_cfg=conv_cfg)
#         self.Block = nn.Sequential(LayerDict)
#
#     def forward(self, x):
#         return self.Block(x)
