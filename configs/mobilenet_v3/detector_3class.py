model = dict(
    ...
    backbone=dict(
        ...
        # stem_channels=12,
        # stage_channels=(12, 24, 32, 42),
        # stem_channels=16,
        # stage_channels=(16, 24, 32, 48),
        # stem_channels=24,
        # stage_channels=(24, 32, 48, 64),
        kernel_size=[3, 3, 3, 3],
        block_per_stage=(1, 3, 3, 1),
        num_out=1,
        conv_type='RepVGGBlock'
        # conv_type='Conv2d',
        # conv_type='GhostBlock'
        ),
    ...
    #         dict(type='BatchMixup', alpha=0.1, num_classes=3, prob=0.2)
...
data_root = '/media/traindata/hand_cls_add'
policies = [
...
train_pipeline = [
    ...
    # dict(type='Pad', size=160),
    dict(type='ResizeOrResizePad', size=128, pad_size=160, resize=160, backend='pillow', prob=5/7),
    # dict(
    #     type='RandAugment',
    #     policies=policies,
    #     num_policies=2,
...
test_pipeline = [
    dict(type='LoadImageFromFile'),
    # dict(type='Resize', size=160, backend='pillow'),
    # dict(type='Pad', size=160),
    dict(type='ResizeOrResizePad', size=128, pad_size=160, resize=160, backend='pillow', prob=5/7),
    dict(type='Normalize', **img_norm_cfg),
    dict(type='ImageToTensor', keys=['img']),
...
resume_from = None
workflow = [('train', 1)]
work_dir = './work_dirs/detector_3class_real_test/3_repvgg_0.25_repvggnet_1113_1311_nopad_AdamW_0.005_keypoint_weight_151_no_repvgg_resize_or_resize_pad_add_211'
